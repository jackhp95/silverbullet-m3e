...
// SpaceForm,UsersView}.tsx, client/spaces_ui/space_fields.tsx — all 12
// call sites, `grep -rn "<Alert" client/spaces_ui`) renders it as a
...
// the field/section it's about to a floating bottom-of-screen overlay. Two
// mitigations kept it from being a worse regression, both explicit here
// rather than assumed:
//   - `duration={0}` (real, documented: SnackbarElement's own duration
//     handling — decompiled snackbar.js's `if (this.duration > 0)
//     setTimeout(...)` — a duration of exactly 0 sets no auto-dismiss timer
//     at all) preserves the original's PERSISTENCE: it does not vanish on a
//     timer while the underlying error is still true, matching `.sb-alert`.
//   - `dismissible` gives the user their own explicit close affordance,
//     which the old bare `<div>` never had.
...
// renders BOTH an error and a warning `<Alert>` in the same view (lines 299
// and 352 of that file), and only one snackbar can ever be visibly open at
// once, so simultaneous error+warning would silently drop one.
