...
test.describe("Guide: Knowledge Base", () => {
  ...
  test("clicking a wiki link navigates to the linked page", async ({
    sbPage,
    sbServer,
  }) => {
    ...
    const ownershipLink = editor.locator(".sb-wiki-link", {
      hasText: "Ownership",
    });
  ...
  test("Ownership page surfaces Rust as a Linked Mention", async ({
    sbPage,
    sbServer,
  }) => {
  ...
  test("hashtag at the top of a body tags the page", async ({
    sbPage,
    sbServer,
  }) => {
    const editor = sbPage.locator("#sb-editor .cm-content");
    ...
    await sbPage.keyboard.press("Enter");
    await sbPage.keyboard.type("Atomic notes are focused on a single idea.", {
      delay: 20,
    });
  ...
  test("query over a frontmatter tag returns matching pages", async ({
    sbPage,
    sbServer,
  }) => {
