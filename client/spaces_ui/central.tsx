...
function CentralLogin({ attempt }: { attempt: string }) {
  ...
  async function signIn(values: LoginValues, provider: boolean) {
    if (!context) return;
    const management = /^\/\.spaces(?:\/|$)/.test(
      new URL(context.destination).pathname,
    );
    const popup =
      !provider && values.clientEncryption && !management
        ? window.open("about:blank", "_blank", "popup,width=600,height=720")
        : null;
    if (!provider && values.clientEncryption && !management && !popup) {
      setError(
  ...
  return (
    <LoginForm
      ...
      clientEncryptionHint={
        /^\/\.spaces(?:\/|$)/.test(new URL(context.destination).pathname)
